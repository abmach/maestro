# Builds the distributable PowerShell module (dist/module/Maestro/) from the
...
if (-not $OutputDir) { $OutputDir = Join-Path $Root "dist/module/Maestro" }
...
$manifest = @"
@{
    RootModule           = 'Maestro.psm1'
    ModuleVersion        = '$Version'
    GUID                 = 'b7e3a9c4-5d18-4f6a-9c2e-1a0d8f4b7e33'
    Author               = 'Arthur Biazon Machado'
    Description          = 'Maestro agent-skills bundle: plan/implement/test/document workflow installer and validators.'
...
"@
Set-Content -LiteralPath (Join-Path $OutputDir "Maestro.psd1") -Value $manifest -Encoding UTF8

$psm1 = @'
# Maestro bundle distribution module. Thin wrappers over the canonical scripts
...
function Install-Maestro {
    ...
    & (Join-Path $script:BundleRoot "Install-Maestro.ps1") @PSBoundParameters
}
...
'@
Set-Content -LiteralPath (Join-Path $OutputDir "Maestro.psm1") -Value $psm1 -Encoding UTF8
