...
- MaestroKit wrapper help blocks moved **above** their functions and given Audience notes — `Get-Help -Full` now renders the NOTES/EXAMPLES sections for Install-Maestro, Test-MaestroKit and Invoke-MaestroSmokeInstall (inside-function placement silently dropped them on current PowerShell builds).
